...
try:
    from funasr.metrics.compute_acc import compute_accuracy  # type: ignore # pyright: ignore[reportMissingImports]
except (ImportError, ModuleNotFoundError):
    def compute_accuracy(preds, target, ignore_label=-100):
        return 0.0
try:
    from funasr.register import tables  # type: ignore # pyright: ignore[reportMissingImports]
    from funasr.train_utils.device_funcs import force_gatherable, to_device  # type: ignore # pyright: ignore[reportMissingImports]
    from funasr.utils.datadir_writer import DatadirWriter  # type: ignore # pyright: ignore[reportMissingImports]
    from funasr.utils.load_utils import extract_fbank, load_audio_text_image_video  # type: ignore # pyright: ignore[reportMissingImports]
except (ImportError, ModuleNotFoundError):
    class _DummyTables:
        encoder_classes = {}
        adaptor_classes = {}
    ...
    tables = _DummyTables()  # type: ignore
    force_gatherable = None  # type: ignore
    to_device = None  # type: ignore
    DatadirWriter = None  # type: ignore
    extract_fbank = None  # type: ignore
    load_audio_text_image_video = None  # type: ignore
...
class FunASRNano(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if hub == "ms":
            from funasr import AutoModel  # type: ignore # pyright: ignore[reportMissingImports]
